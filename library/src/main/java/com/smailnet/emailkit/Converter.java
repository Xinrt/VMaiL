...
class Converter {
        ...
        static EmailKit.Config getMailConfiguration(String type) {
            switch (type) {
                ...
                case EmailKit.MailType.OFFICE365:
                    return new EmailKit.Config()
                            .setSMTP("smtp.office365.com", 587, false, true)
    ...
    private static class DateUtils {
        ...
        static Message.SentDate getDateInfo(Date date) {
            if (date != null) {
                SimpleDateFormat format = new SimpleDateFormat("d-MMMM-yyyy hh:mm", Locale.ENGLISH);
    ...
    private static class ContentUtils {
        ...
        static List<Message.Content.Attachment> getAttachmentList(Part part) throws IOException, MessagingException {
            ...
            for (int i = 0; i < bodyPartList.size(); i++) {
                ...
                String filename = generateSuffix() + TextUtils.decodeText(bodyPart.getFileName());
        ...
        static class MessageUtils {
            ...
            static MimeMessage toInternetMessage(EmailKit.Config config, Draft draft) {
                try {
                    ...
                    //如果是一封回复邮件
                    if (draft.getReplyDraft()) {
                        // 发送网络请求获取到要回复的这封邮件的信息
                        IMAPStore store = EmailUtils.getStore(config);
                        IMAPFolder folder = EmailUtils.getFolder(draft.getReplyFolderName(), store, config);
                        javax.mail.Message msg = folder.getMessageByUID(draft.getReplyUid());
                        if (msg != null) {
                            message = (MimeMessage) msg.reply(draft.getReplyToAll());
                        } else {
                            throw new RuntimeException("Cannot reply now");
                        }
                    }
                    //判断是否存在抄送人
                    if (draft.getCc() != null) {
